...
var iPath = "";
function Node(id, pid, name, url, title, target, open, pk, param, num, icon, iconOpen) {
	this.num = num;
	this.id = id;
	this.pid = pid;
	this.name = name;
...
dTree.prototype.node = function (node, nodeId) {
	...
	if (node.url) {
		...
		str += "><font id=\"tr2\" name=\"" + node.title + "\">";
	} else {
		if ((!this.config.folderLinks || !node.url) && node._hc && node.pid != this.root.id) {
			str += "<a href=\"javascript: " + this.obj + ".o(" + nodeId + ");\" class=\"node\"><font id=\"tr2\" name=\"" + node.id + "\">";
		}
	}
	str += node.name;
